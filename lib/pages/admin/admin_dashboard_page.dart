...
// ═══════════════════════════════════════════════════════════════════════════
//  Admin Dashboard – entry / access gate
// ═══════════════════════════════════════════════════════════════════════════
...
Widget _statCard(BuildContext ctx, String label, String value, IconData icon,
    Color color) {
...
Widget _loadingOrError(BuildContext ctx, AsyncSnapshot snap) {
  ...
  if (snap.connectionState == ConnectionState.waiting) {
    return Center(
        child: SpinKitFadingCube(color: theme.primary, size: 36));
  }
  return Center(
...
class _OverviewTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<List<QuerySnapshot>>(
      ...
      builder: (ctx, snap) {
        ...
        // Rating breakdown from contractors
        final ratingBuckets = [0, 0, 0, 0, 0]; // 1-star thru 5-star
        int ratedCount = 0;
        for (final c in contractors) {
          final avg = ((c.data() as Map)['rating_avg'] as num?)?.toDouble() ?? 0;
          if (avg > 0) {
            ratedCount++;
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Card(
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                    _buildStatusBar(ctx, 'Pending',
                        statusCounts['pending'] ?? 0, orders.length,
                        const Color(0xFFFF9800)),
                    const SizedBox(height: 10),
                    _buildStatusBar(ctx, 'Confirmed',
                        statusCounts['confirmed'] ?? 0, orders.length,
                        const Color(0xFF2196F3)),
                    const SizedBox(height: 10),
                    _buildStatusBar(ctx, 'Partially Paid',
                        statusCounts['partially_paid'] ?? 0, orders.length,
                    ...
                    const SizedBox(height: 10),
                    _buildStatusBar(ctx, 'Cancelled',
                        statusCounts['cancelled'] ?? 0, orders.length,
  ...
  Widget _buildStatusBar(BuildContext ctx, String label, int count, int total,
      Color color) {
    final theme = FlutterFlowTheme.of(ctx);
...
class _ReviewsTabState extends State<_ReviewsTab> {
  ...
  Future<List<_ReviewEntry>> _fetchAllReviews() async {
    ...
    // Reviews are stored in top-level 'reviews' collection with a contractor_ref field
    ...
    for (final r in reviewsSnap.docs) {
      final rData = r.data();
      final contractorRef =
          rData['contractor_ref'] as DocumentReference?;
      ...
      try {
        ...
        if (cSnap.exists) {
          contractorName =
              (cSnap.data() as Map?)?['display_name'] as String? ??
                  'Contractor';
        }
      } catch (_) {}
  ...
  Future<void> _deleteReview(
      BuildContext ctx, _ReviewEntry entry) async {
    final theme = FlutterFlowTheme.of(ctx);
    ...
    try {
      await FirebaseFirestore.instance
          ...
          .doc(entry.reviewId)
          .delete();
      // Recalculate rating on contractor doc
          ...
          builder: (ctx, snap) {
            ...
            if (_query.isNotEmpty) {
              docs = docs.where((d) {
                final data = d.data() as Map<String, dynamic>;
                final a =
                    (data['user_a_name'] as String? ?? '').toLowerCase();
                final b =
                    (data['user_b_name'] as String? ?? '').toLowerCase();
          ...
          builder: (ctx, snap) {
            ...
            if (_query.isNotEmpty) {
              docs = docs.where((d) {
                ...
                final provider = (data['provider_name'] as String? ?? '')
                    .toLowerCase();
                final title =
...
class _AdminOrderCard extends StatelessWidget {
        ...
        Padding(
          ...
          child: Column(
              ...
              children: [
                ...
                Row(children: [
                  _infoChip(
                      context, Icons.person_outline_rounded, clientName),
                  const SizedBox(width: 8),
                  Icon(Icons.arrow_forward_rounded,
                      size: 14, color: theme.secondaryText),
                  const SizedBox(width: 8),
                  _infoChip(context, Icons.build_circle_outlined,
                      providerName),
                ]),
                const SizedBox(height: 8),
                Row(children: [
              ...
              child: ListView(
                ...
                children: [
                  ...
                  _detailSection(theme, 'Service Information', [
                    _detailRow(theme, Icons.work_outline_rounded,
                        'Title', title),
                    if (description.isNotEmpty)
                      _detailRow(theme, Icons.description_outlined,
                          'Description', description),
                    _detailRow(theme, Icons.schedule_rounded,
                        'Delivery', '$deliveryDays day${deliveryDays == 1 ? '' : 's'}'),
                    _detailRow(theme, Icons.calendar_today_rounded,
                  ...
                  _detailSection(theme, 'Parties', [
                    _detailRow(theme, Icons.person_rounded,
                        'Client', clientName),
                    _detailRow(theme, Icons.build_circle_outlined,
                  ...
                  _detailSection(theme, 'Payment Summary', [
                    _detailRow(theme, Icons.payments_rounded,
                        'Total Amount', '$currency ${amount.toStringAsFixed(3)}'),
                    if (amountPaid > 0)
                      _detailRow(theme, Icons.check_circle_outline_rounded,
                          'Amount Paid',
                          '$currency ${amountPaid.toStringAsFixed(3)}',
                          valueColor: const Color(0xFF4CAF50)),
                    if (remaining > 0 && amountPaid > 0)
                      _detailRow(theme, Icons.hourglass_bottom_rounded,
                          'Remaining',
                  ...
                  // ── Installment plan (only if applicable) ──
                  if (isInstallment) ...[
                    _detailSection(theme, 'Installment Plan', [
                      _detailRow(theme, Icons.calendar_month_rounded,
                          'Total Months',
                          '$installmentsTotal month${installmentsTotal == 1 ? '' : 's'}'),
                      _detailRow(theme, Icons.done_all_rounded,
                          'Months Paid', '$installmentsPaid of $installmentsTotal'),
                      _detailRow(theme, Icons.pending_actions_rounded,
                          'Months Remaining',
                          '${installmentsTotal - installmentsPaid} left'),
                      _detailRow(theme, Icons.monetization_on_rounded,
                          'Per Month',
  ...
  Widget _detailSection(FlutterFlowTheme theme, String title,
  ...
  Widget _detailRow(FlutterFlowTheme theme, IconData icon, String label,
      String value,
      {Color? valueColor}) {
    return Padding(
      padding: const EdgeInsets.only(bottom: 10),
