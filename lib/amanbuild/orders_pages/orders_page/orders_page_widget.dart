          ...
          body: ConnectivityWrapper(
            child: Column(
              children: [
                ...
                Expanded(
                  child: isProvider
                      ? const _OrdersTab(queryField: 'provider_uid', isProvider: true)
                      : const _OrdersTab(queryField: 'client_uid', isProvider: false),
                ),
              ],
            ),
          ),
...
class _OrderCard extends StatelessWidget {
    ...
    if (!isProvider) {
      ...
      if (status == 'confirmed' || status == 'partially_paid') {
        ...
        final payLabel = isInstallment
            ? 'Pay Month $nextMon: ${_kFmt(nextAmount)} $_kCurrency'
  ...
  Future<void> _markMonthComplete(
    ...
    try {
      ...
      if (context.mounted) {
        _orderSnack(context, FlutterFlowTheme.of(context),
            'Month $monthNum marked complete ✅', isError: false);
      }
    } catch (e) {
